...
### ★ 문서 작성 규율 — 내가 정하지 않은 것을 규칙으로 쓰지 마라

**문서에는 사용자가 실제로 말한 것만 결정으로 적는다.** 모델이 채워 넣은 추론을
결정처럼 적으면 안 된다.

실제로 난 사고 (2026-09-02 → 09-08에 발각):

- `Docs/LEVEL_DESIGN.md`에 단차 3 / 갭 5 / 통로 2칸, "발판 위에 안전지대를 만들지 마라",
  "보스 방은 발판 최소한", "북문은 천장 정중앙", "보상은 선택, 통행은 보장" 같은 규칙이
  적혔다. **전부 모델이 정한 것이고 사용자는 승인한 적이 없다.**
- 그런데 EditMode 테스트 353개가 그것을 이미 강제하고 있었다.
- 더 나쁜 건, 나중 세션에서 모델이 **자기가 쓴 문장을 "특수방 규칙"이라며 사용자에게
  근거로 인용**했다. 추론이 문서를 거쳐 결정으로 세탁되는 경로가 생긴 것이다.

**지켜야 할 것**

1. **출처를 구분해서 쓴다.** 사용자가 정한 것과 모델이 제안한 것을 같은 문서에
   같은 어조로 섞지 않는다. 모델 제안은 "제안", "미결", "확인 필요"로 명시한다.
2. **미결은 `Docs/OPEN_QUESTIONS.md`로.** 결정된 것만 `DESIGN.md` 결정 로그에 넣는다.
3. **문서를 근거로 인용하기 전에 출처를 확인한다.** 특히 커밋되지 않은 문서는
   승인받지 않았을 가능성이 높다 (`git log --oneline -- <파일>`로 확인).
4. **테스트로 규칙을 못 박기 전에 그 규칙이 누구 것인지 확인한다.**
   테스트는 문서보다 강한 강제력을 갖는다.
5. 사용자가 언급하지 않은 내용을 임의로 문서에 추가하지 않는다. 필요하다고 판단되면
   문서에 넣지 말고 **대화에서 먼저 물어본다.**
